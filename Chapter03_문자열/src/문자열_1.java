 ...
 * 		=> toLowerCase() : 소문자 변환
 * 	***	=> trim() : 공백문자 제거 (좌우의 공백)
 * 		=> split() : 지정한 문자로 문자열을 나누는 경우
 * 		=> replaceAll() : 정규표현식(가~힣/A~Z/a~z/0~9)을 지정한 문자로 바꿔서 출력
 * 	***	=> valueOf() : 지정한 개체의 원시 값을 반환 => 정수/실수/문자/논리 => 문자열로 변환
 * 		=> toString() : 지정한 개체의 원시 값을 반환 => null : 아예 없을 수 있다
 * 		~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 반드시 암기
