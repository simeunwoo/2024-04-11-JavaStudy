...
public class EmpMemberDAO {
	   ...
	   public ArrayList<EmpVO> empListData(int page)
	     ...
	     // 검색 => LIKE 
	     public ArrayList<EmpVO> empFindData(String name, String option){
	    	 ArrayList<EmpVO> list = new ArrayList<EmpVO>();
	    	 try {
				getConnection();
				String sql = "SELECT empno , ename ,job,sal,hiredate,dname,loc,performance"+
							" FROM emp,dept "+
						    " WHERE emp.deptno = dept.deptno and " + option + " like '%'||?||'%' "+
							" ORDER BY empno ASC";
	     ...
	     public EmpVO empDetailData(String no) {
	 		...
	 		try {
	 			...
	 			// 데이터 읽기
	 			String sql = "SELECT empno ,ename, job ,sal ,hiredate,performance ,(select dname from dept where dept.deptno = emp.deptno) , "
	 					+ "	(select loc from dept where dept.deptno = emp.deptno ) , "
	 					+ "	 (select deptno  from dept where dept.deptno = emp.deptno ) " + "	 FROM emp "
	 					+ "	 WHERE empno=?";
