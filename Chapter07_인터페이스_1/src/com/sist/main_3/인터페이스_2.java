 ...
 * 		---------------------
 * 		메소드 : 구현이 안된 메소드
 * 		[접근지정어] abstract 리턴형 메소드형(매개 변수...)
 * 		=> 추상 메소드 : 상속 받은 클래스마다 기능은 동일 / 구현 내용이 다른 경우
 * 		---------------------
 * 		자바 => 추상 : 공통적인
 * 	}
 * 	=> 관련된 클래스를 묶어서 관리
 * 
 * 	인터페이스 : 추상 클래스 보완 => 추상 클래스의 일종
 * 		=> 설계와 관련 => 개발 기간 단축
 * 		=> 표준화 => 모든 개발자가 같은 메소드를 사용한다
 * 		=> 독립적으로 사용 가능
 * 		=> 클래스를 묶어서 관리
 * 		=> 결합성이 낮은 프로그램 => 에러 시 다른 클래스에 영향이 없다
 * 		
 * 		1. 윈도우 : 이벤트 처리 : 버튼 클릭, 마우스 클릭, 키보드 입력 ...
 * 		2. 컬렉션 : 데이터를 모아서 쉽게 관리 => 자료 구조
 * 		3. 데이터베이스 : 연결, 닫기, SQL 문장 ...
 * 		4. 웹 프로그램에서 => 요청 받기, 요청 처리, 결과값 전송
 * 		=> 확인 => new를 사용하지 않고 객체를 생성하면 인터페이스
 ...
 * 			---------------------------
 * 			상수 => 변수는 존재 X
 * 			(public static final) 데이터형 상수명=값 // 상수명=값 : 명시적인 초기화
 * 			---------------------------
 * 			구현이 안된 메소드 : 핵심
 ...
 * 			}
 *
 *			(public) static 리턴형 메소드명(매개 변수...)
 *			{
 *			}
 *			=> default / static => 구현이 가능 => JDK 1.8 이상에서만 가능
 *			=> 유지 보수 => 전에 개발이 된 경우에는 사용할 수 없다
 *			=> 권장 : 메소드 선언만 한다
 * 			---------------------------
 * 		}
 * 		접근지정어 : 다른 클래스와 연결 => public => 생성자가 없다 => 변수가 없다 (상수만 존재)
 * 
 * 		인터페이스도 상위 클래스 기념
 ...
 * 			interface B
 * 			class C
 * 			class D (클래스 상속) (인터페이스 상속) => class D extends C implements A,B // 클래스 상속 먼저 코딩해야 함
 * 
 * 	내부 클래스 => 네트워크, 게임
