 ...
 * 									  --------------------- 메소드
 * 	String 사용법
 * 	String name"ASSS"; // 글자수 제한이 없다 => char[]을 클래스화
 * 	------------------
 * 	int (데이터형) => Integer (클래스형)
 * 	long => Long
 * 	byte => Byte
 * 	double => Double
 * 	boolean => Boolean
 ...
 *		= replaceAll()
 *		= split()
 *		= concat() => 문자열 결합 시 사용 ('+'가 이미 있어서 요즘 거의 사용 안한다)
 *	   제어
 *		= substring()
 *		= indexOf()
 *		= lastIndexOf()
 ...
 *	배열
 *	String[] 배열명={} => 데이터를 고정
 *	String[] 배열명=new String[10]; => 값 초기화, 값 변경,...
 */
...
public class 문자열배열_1 {
	...
	public static void main(String[] args) {
		...
		// 노래 출력
		System.out.println("등록된 곡 총 "+title.length); // 등록된 곡 총 50
		int rank=1;
		for(String t:title)
		{
			System.out.println(rank+". "+t); // 1. Your Lights...50. Blueming
			rank++;
		}
		System.out.println("~~~~~~~~~~~~~~~~");
