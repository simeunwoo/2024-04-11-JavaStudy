...
public class StudentDAO {
	...
	/*
	 * 	CallableStatement : 프로시저 호출 시에 사용
	 * 		= SELECT, INSERT, UPDATE, DELETE => executeQuery()만 사용
	 * 	PreparedStatement : 일반 SQL 문장 전송 사용
	 * 		= executeQuery() => SELECT 문장으로 실행 결과값을 가지고 올 때 사용
	 * 		= executeUpdate() => 오라클에 있는 데이터 변경 (INSERT, UPDATE, DELETE)
	 */
	private final String URL="jdbc:oracle:thin:@localhost:1521:XE";
	private static StudentDAO dao; // 싱글턴 : 한개의 메모리에 생성된 객체 => 재사용 목적 => 메모리 누수 방지
	// 드라이버 등록
	public StudentDAO()
	{
		try
	...
	public List<StudentVO> studentAllData()
	{
		...
		{
			getConnection();
			String sql="{CALL studentAllData(?)}"; // 프로시저 호출
			cs=conn.prepareCall(sql);
			// ?에 값을 채운다 => IN 변수는 기존과 동일, OUT 변수는 저장하는 공간을 지정
	...
	*/
	...
	END;
	*/
	// 프로시저는 DBA가 보통 제작 => 웹 프로그래머는 자바로 프로시저 호출이 가능
	// 프로시저 => 댓글
