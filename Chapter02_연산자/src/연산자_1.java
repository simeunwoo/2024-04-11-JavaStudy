 ...
 * 				boolean b=true
 * 				=> !b = false => 조건 => 로그인, 검색어,...
 * 			> 형변환연산자 : 모든 데이터형은 변경이 가능 (boolean 제외)
 ...
 * 				&& => 직렬연산자 (범위나 기간 안에 있는 경우) => ex) 예약, 체크인/체크아웃,...
 * 				|| => 병렬연산자 (범위나 기간을 벗어난 경우)
 ...
 * 		int + int = int
 * 		long + long = long
 * 		int + long = long (여기서 왼쪽의 int는 사전에 long으로 변경 후에 연산)
 * 							=> (데이터형을 큰 형으로 변경 후에 연산)
 * 		char + int = int (int보다 작은 데이터형은 연산 시에 결과값이 int)
 * 		char + char = int (")
 * 		byte + byte = int (")
 * 		byte + char = int (")
 * 		int + double + char = double => 85.5 (결과값은 가장 큰 데이터형이 나온다)
 * 		10	   10.5   'A'(=65)
 * 		
 * 		*** +, -, * (동일)
 * 		*** /
 * 			=> 0으로 나누면 오류 발생 => 10/0 => 오류 (0으로 나눌 수 없다)
 * 			=> 정수/정수 => 정수 (실수점 제외) => 소수점 필요 시 정수/실수
 * 				5/2		  2 (2.5가 아니다)
 * 		*** %
 * 			=> 나누고 나머지값
 * 			=> 5%2 => 1 => 배수
 ...
 * 			   ------------ 항상 왼쪽 부호를 따라 간다
 * 
 * 	*** 데이터형의 크기 : byte < char < int < long < float < double (boolean은 제외)
 */
public class 연산자_1 {
	...
	public static void main(String[] args) {
		...
		System.out.println("국어 점수:"+kor); // 국어 점수:80
		System.out.println("영어 점수:"+eng); // 영어 점수:75
		System.out.println("수학 점수:"+math); // 수학 점수:90
		System.out.println("총점:"+total); // 총점:245
		System.out.println("평균:"+avg); // 평균:81.66666666666667
		System.out.printf("평균:%.2f",avg); // 평균:81/67 => %.2f => 소수점 아래 2자리까지
											// 이 때 +avg가 아니라 avg에 앞에 ,를 사용한다
	}
