 ...
 * 
 * 		메소드
 * 		= 인스턴스 메소드 : new에 의하여 따로 저장 *****
 * 	 	= 공통 메소드 : static => 웹 (데이터베이스 연동, 한글 변환, 암호화, 복호화)
 ...
 * 			return ...
 * 		}
 * 		리턴형 : 사용자 요청 처리 결과 => 목록, 상세 보기, 총 페이지 ...
 * 		매개변수 : 사용자가 요청한 내용 => id, pwd, 검색어 ...
 * 		*** 메소드는 호출 시마다 처음부터 실행하여 메소드가 종료가 되는 원래 호출된 위치로 돌아온다
 * 		*** 메소드 사용 이유
 * 		=> 반복 제거 (반복적인 코딩) => 반복문 가능 => 메소드 => 코드의 간소화
 * 		=> 관련된 명령문 (입력/출력)을 모아서 관리 => 가독성 (구조화된 프로그램) => 기능 분리
 * 		=> 재사용 (수정, 추가)
 * 
 * 		=> 호출
 * 		멤버변수 / 메소드
 ...
 * 			메소드 => 인스턴스 메소드
 * 					다른 클래스와 연결 => public
 * 			생성자 => 필요 시에만 사용 => 자동으로 추가 : public
 * 			--------------------------------- 웹사이트 => 변수 추출 / 기능 추출 (ex) 메뉴) => 요구사항 분석
 * 
 * 		어떤 데이터를 사용자에게 보여줄지 => 멤버 변수
 * 		데이터를 보여주기 위하여 어떤 기능을 설정할지 => 멤버 메소드
