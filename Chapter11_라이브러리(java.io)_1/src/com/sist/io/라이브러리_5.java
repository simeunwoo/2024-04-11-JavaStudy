 ...
 * 				System.in : 입력 => read()
 * 				---------- 예외 처리가 필요, read() => 문자의 번호 (int) => 형변환이 필요해서 불편함
 * 		=> 파일 입출력
 * 		=> 네트워크 입출력
 * 		=> 단점 : 단방향 통신을 한다 => 읽기/쓰기 따로 만들어진다 => 빨대 느낌 (단방향)
 * 			=> 만약 양방향으로 통신하고 싶다면? => 프로그램을 두개 동작 (읽기/쓰기) => 쓰레드 (네트워크)
 */
// 파일 입출력
...
//		FileInputStream fis=null; // FileInputStream을 사용할 경우 => 1byte라서 한글이 깨진다 => 한글은 2byte가 필요
		FileReader fis=null; // 한글 포함 텍스트를 제어
		try
		{
			File file=new File("C:\\Users\\sist115_1\\git\\java\\Chapter11_라이브러리(java.io)_1"
					+ "\\src\\com\\sist\\io\\라이브러리_1.java");
		...
		{
			// 정상 수행 (try) / 오류 발생 (catch) 상관 없이 무조건 수행하는 문장 (finally)
			try
			{
				fis.close();
			}catch(Exception e) {}
