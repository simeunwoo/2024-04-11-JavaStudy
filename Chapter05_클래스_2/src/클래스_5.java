/*
 * 	클래스의 구조 => 프로그램에 맞게 데이터/메소드를 관리하기 위한 설계
 * 	=> 활용은 메모리에 저장 후에 사용
 * 			========= new 생성자() => 받은 주소를 이용한다
 * 									-------------- 객체
 * 
 ...
 * 		=> 기타 변수 => 지역 변수로 활용
 * 
 * 	멤버 변수 : new를 사용해서 저장이 가능
 * 			클래스 전체 / 다른 클래스에 사용 가능
 * 			일반 변수와 동일
 * 			=> 클래스는 한개에 대한 정의
 * 				=> 영화 1개 / 레시피 1개 => 저장만 여러개
 * 			*** 공통변수가 사용되는 변수는 static을 이용한다
 * 
 * 	생성자 : 클래스명과 동일 / 리턴형이 없다
 * 			=> 역할 메모리 할당할 때 호출되는 메소드
 * 			=> 멤버 변수의 초기화
 * 			   ------------- 서버 구동 / 화면 UI / 데이터베이스 연동
 * 			=> 반드시 필요하다 => 경우에 따라서 생략 => 컴파일러가 자동으로 생성자를 추가해준다
 * 	메소드 : 멤버 변수가 가지고 있는 데이터를 활용할 수 있게 기능을 추가
 * 	-------------------------------------------------- 클래스의 구성 요소
 * 	*** 다른 클래스에서 사용 가능
 * 		메소드 => 다른 클래스와 통신 (연결)
 * 		---- 기능 변경 / 추가
 ...
 * 		1) 캡슐화 : 데이터 보호 목적 => 파밍 => 크롤링(X) => 해킹 => Vue/React/Ajax
 * 		2) 재사용 : 상속, 포함 : 있는 그대로 사용
 * 				  --------
 * 					| 변경해서 사용
 ...
 * 	---------------------------------------- 통합 : Spring
 * 												  ------- 자바 기반
 * 	*** 프로그램이 비정상 종료가 안되게 만든다 : 예외 처리 (8장) => 자바 기본 문법
 */
class Movie
{
	int rank;
...
public class 클래스_5 {
	...
	public static void main(String[] args) {
		...
		Movie m2=new Movie();
		System.out.println("m2="+m2); // 2=Movie@e73f9ac
		m2.rank=2;
		...
		Movie m3=new Movie();
		System.out.println("m3="+m3); // m3=Movie@61064425
		m3.rank=3;
		m3.title="악마와의 토크쇼";
		m3.reserve="15.2%";
